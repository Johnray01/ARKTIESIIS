...
1. The institution confirmed this field-to-type mapping: report card—student name, school name, and grades; Good Moral Certificate—student name and school name; PSA birth certificate—student name. The current capstone implementation surfaces linked student-name text for each digital type, possible school-name lines for report cards and Good Moral Certificates, and apparent grade-entry lines for report cards as bounded, escaped suggestions requiring source inspection. This mapping does not define pass/fail criteria.
...
- Leader-decided role, advisory, manual-decision, override-reason, immutable-history, concurrency, and Form 137 transient-scan/status behavior is covered by tests.
- The confirmed field mapping does not specify name-matching rules (including treatment of middle names, aliases, or OCR errors), accepted school-name variants or formats, what grade entries/periods are required, document completeness rules, or format/compliance criteria. Those details and any grades threshold or school-name whitelist need explicit institutional approval before they become pass/fail checks.
- The institution has not supplied the exact review cases and student-facing wording for verified, correction requested, and rejected outcomes. Current labels are operational UI actions only; approved decision criteria and exact messages remain a policy gate. No school approval is inferred from this capstone implementation.
...
Phase 15 is the time to prepare and exercise the deployment runbook: confirm the VPS plan and license, provision plain Ubuntu 22.04, harden SSH and the host firewall, install Node.js 20+ and the native Tesseract/Poppler tools, configure the app as a managed service behind HTTPS, keep SQL Server unavailable to the public network, configure secrets outside the repository, and place persistent uploads and verified backups on private durable storage. Install a supported SQL Server 2022 CU (CU 10 or later) with an appropriately licensed production edition, then run migrations only through the documented forward-only setup against the prepared production database. Verify backup restore, role access, private file delivery, and real Linux OCR on the VPS. Do not treat a local Developer container, a smoke test, or a plan description as production deployment evidence. No VPS provisioning, database migration, license purchase, or production deployment is part of the earlier phases.

**Acceptance gate:** the agreed Hostinger KVM 2/Ubuntu 22.04 target is documented and its deployed configuration is verified; production settings require secrets from the environment and do not permit development-only authentication or seed operations; production SQL Server is a supported Ubuntu 22.04 CU 10+ build with production-appropriate licensing; uploaded files remain private and persistent across restarts; real native OCR passes on Linux; backup and restore steps have been exercised; the demo seeder is explicitly guarded and safe to rerun; the final demo follows the approved role/document boundaries and has a written checklist.

## Stop conditions

- **Goal 1:** Run Phases 8–10 in order within one Goal Mode objective. After each phase passes its acceptance gate, report a concise checkpoint and create one local commit for that completed phase. Phase 9 requires the agreed local checks and a successful real native OCR smoke on Linux. Windows path-handling tests and the historical Windows report are compatibility evidence only, not an alternate or additional runtime gate. Capstone leader-directed advisory/manual-review scaffolding may be implemented when requested. Keep policy-dependent Phase 10 checks on hold until approved required fields, format rules, review cases, and decision wording are supplied; do not mark Goal 1 complete while that gate is pending. Never push. Stop after Phase 10 passes its full gate and report Goal 1 complete. Do not start Phase 11 or any later phase in this goal.
